...
$ts2 = addSpan($ts,'2d');
$iso2 = fromTS2ISO($ts2);
cmp_ok($iso2, 'eq', '20190417','moving to two dayes later'); #test 3

#moving to two days before
$ts2 = subSpan($ts,'2d');
$iso2 = fromTS2ISO($ts2);
cmp_ok($iso2, 'eq', '20190413','moving to two dayes before'); #test 4

#time trasformation tests
my $time = timelocal(0,0,0,15,9,1963);
my $Ts = fromTS2ISO($time);
cmp_ok('19631015','eq',$Ts,'time formatting test'); #test 5

#add 7 days
my $then = addSpan($time,'7d');
my $thenTs = fromTS2ISO($then);
cmp_ok('19631022','eq',$thenTs,'add seven days'); #test 6

#add 4 days near year end
my $yearEnd = timelocal(0,0,0,30,11,2012);
$then = addSpan($yearEnd,'4d');
$thenTs = fromTS2ISO($then);
cmp_ok('20130103','eq',$thenTs,'add four days near year end'); #test 7

#add 2 months
$then = addSpan($time,'2m');
$thenTs = fromTS2ISO($then);		
cmp_ok('19631215','eq',$thenTs,'add two months'); #test 8

#add 3 months near year end
$then = addSpan($yearEnd,'3m');
$thenTs = fromTS2ISO($then);		
cmp_ok('20130330','eq',$thenTs,'add three months near year end'); #test 9

#add 1 year
$then = addSpan($time,'1y');
$thenTs = fromTS2ISO($then);		
cmp_ok('19641015','eq',$thenTs,'add one year'); #test 10

#subtracting 7 days
$then = subSpan($time,'7d');
$thenTs = fromTS2ISO($then);		
cmp_ok('19631008','eq',$thenTs,'subtracting seven days'); #test 11

#subtracting 4 days near year start
my $yearStart = timelocal(0,0,0,3,0,2013);
$then = subSpan($yearStart,'4d');
$thenTs = fromTS2ISO($then);		
cmp_ok('20121230','eq',$thenTs,'subtracting four days near year start'); #test 12
...
$thenTs = fromTS2ISO($then);		
cmp_ok('20121003','eq',$thenTs,'subtracting three months near year start'); #test 14

#Subtracting 1 year
$then = subSpan($time,'1y');
$thenTs = fromTS2ISO($then);		
cmp_ok('19621015','eq',$thenTs,'subtracting one year'); #test 15
